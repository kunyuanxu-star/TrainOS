...
const HEARTBEAT_INTERVAL: usize = 500; // ticks between heartbeat cycles

/// Unique cluster node ID (set at boot, typically 0 for the first node).
pub static mut CLUSTER_NODE_ID: u8 = 0;

/// Well-known distributed IPC endpoint base. Node N receives on EP = DIST_EP_BASE + N.
...
static REMOTE_NODES: Mutex<[RemoteNode; MAX_REMOTE_NODES]> = Mutex::new([EMPTY_NODE; MAX_REMOTE_NODES]);
...
/// Initialize the distributed IPC subsystem.
pub fn init(cluster_node_id: u8) {
    ...
    memory::init();
    crate::println!(
        "DIST: distributed IPC initialized (cluster node {})",
        cluster_node_id
    );
}
...
/// Probe a remote node by sending a ping and waiting for pong.
pub fn node_probe(node_id: u32) -> bool {
    ...
    // Send ping via virtual transport to the target node's well-known endpoint.
    ...
    let plen = 6usize;
    msg.payload[..plen].copy_from_slice(&ping_buf[..plen]);
...
/// Check the receive buffer for a pong from the given node.
...
/// Mark a node as online with the current tick.
...
/// Periodic heartbeat tick — called from timer interrupt every 500 ticks.
pub fn heartbeat_tick() {
    let current_tick: usize;
    unsafe {
        current_tick = crate::trap::TICK_COUNT;
        if current_tick.wrapping_sub(LAST_HEARTBEAT_TICK) < HEARTBEAT_INTERVAL {
            return;
        }
    ...
    for i in 0..count {
        ...
        if nodes[i].online
            && current_tick.wrapping_sub(nodes[i].last_seen_tick) < HEARTBEAT_INTERVAL * 3
        {
            continue;
        }
        ...
        // Send ping
        let mut ping_buf = [0u8; 64];
        if write_ping(&mut ping_buf, nodes[i].node_id).is_none() {
            continue;
        }
        ...
        if !nodes[i].online {
            ...
            if nodes[i].failed_pings >= 3 {
                crate::println!(
                    "DIST: node {} marked offline ({} failed pings)",
                    nodes[i].node_id,
                    nodes[i].failed_pings
                );
            }
        }
    }
...
/// Handle an incoming ping — send a pong back.
fn handle_incoming_ping(src_node: u32) {
    ...
    let target_ep = DIST_EP_BASE + src_node as usize;
    endpoint::send(target_ep, 0, msg).ok();
}

/// Handle an incoming pong — update node status.
fn handle_incoming_pong(src_node: u32) {
    let tick = unsafe { crate::trap::TICK_COUNT };
    mark_node_online(src_node, tick);
}
...
pub fn remote_send(node_id: u32, remote_ep: usize, data: &[u8]) -> Result<(), &'static str> {
    ...
    let written =
        write_data(&mut wire_buf, src_node, remote_ep as u16, data).ok_or("packet too large")?;

    let pid = crate::sched::current_thread()
        .map(|t| unsafe { (*t).owner })
        .unwrap_or(0);

    // Check if this is a virtual node (port==0) or a real remote node
    let is_virtual = {
        let nodes = REMOTE_NODES.lock();
        let count = *REMOTE_NODE_COUNT.lock();
        let mut virt = true;
        for i in 0..count {
            if nodes[i].node_id == node_id && nodes[i].port != 0 {
                virt = false;
            }
        }
        virt
...
/// Queue an incoming remote message into the receive buffer.
...
/// Receive a message from a remote node. Blocks until available.
pub fn remote_recv() -> Result<(u8, u16, [u8; 64], usize), &'static str> {
    loop {
        ...
        for i in 0..count {
            if buf[i].valid {
                let entry = RcvBufEntry {
                    src_node: buf[i].src_node,
                    src_ep: buf[i].src_ep,
                    payload: buf[i].payload,
                    payload_len: buf[i].payload_len,
                    valid: buf[i].valid,
                };
                buf[i].valid = false;
                // Compact buffer
                ...
                for j in 0..count {
                    if buf[j].valid {
                        if compact_idx != j {
                            buf[compact_idx] = RcvBufEntry {
                                src_node: buf[j].src_node,
                                src_ep: buf[j].src_ep,
                                payload: buf[j].payload,
                                payload_len: buf[j].payload_len,
                                valid: true,
                            };
                            buf[j].valid = false;
                        }
                ...
                *RCV_BUF_COUNT.lock() = compact_idx;
                return Ok((entry.src_node, entry.src_ep, entry.payload, entry.payload_len));
            }
        }
        drop(buf);
        crate::sched::schedule();
...
/// Serialize a local capability and send it to a remote node.
pub fn remote_mint(node_id: u32, local_cap_slot: u32, remote_cnode: u32) -> Result<(), &'static str> {
    ...
    let sc = local_cap_slot.to_le_bytes();
    res_data[0] = sc[0];
    res_data[1] = sc[1];
    res_data[2] = sc[2];
    res_data[3] = sc[3];
    let rc = remote_cnode.to_le_bytes();
    res_data[4] = rc[0];
    res_data[5] = rc[1];
    res_data[6] = rc[2];
    res_data[7] = rc[3];
    ...
    let written = write_cap(&mut wire_buf, src_node, cap_type, rights, local_cap_slot, &res_data)
        .ok_or("cap serialize failed")?;
    ...
    crate::println!(
        "DIST: remote_mint node={} slot={} cap_type={} rights={}",
        node_id,
        local_cap_slot,
        cap_type,
        rights
...
/// Receive a capability transfer from a remote node.
pub fn cap_receive(cnode_id: usize, buf: &[u8]) -> Result<(), &'static str> {
    let (src_node, cap_type, rights, slot, _res_data) =
        parse_cap(buf).ok_or("invalid cap packet")?;

    if cap_type == 0 {
        return Err("null cap type");
    }
    ...
    let new_slot = crate::cap::types::Slot {
        cap_type: ktype,
        rights,
        resource_id,
    };
    ...
    if let crate::cap::types::ResourceData::CNode { ref slots } = &res.data {
        ...
        while s.len() <= idx {
            s.push(crate::cap::types::Slot::null());
        }
        s[idx] = new_slot;
    }

    crate::println!(
        "DIST: cap_receive from node={} type={} rights={} slot={}",
        src_node,
        cap_type,
        rights,
        slot
...
/// Copy a cap from a local slot and forward to a remote node.
...
pub use memory::{
    ...
    print_pool_stats, get_available_pages, remote_alloc_pages, register_mem_node,
    set_available_pages,
};

// ── 26.3 Cluster Coordination ───────────────────────────────────────────────────

/// Get the cluster node ID of the current process.
pub fn get_cluster_node_id() -> u8 {
    unsafe { CLUSTER_NODE_ID }
}

/// Encode a global PID from (node_id, local_pid).
pub fn encode_global_pid(node_id: u8, local_pid: u32) -> u32 {
    ((node_id as u32) << 24) | (local_pid & 0x00FF_FFFF)
}

/// Extract the cluster node ID from a global PID.
pub fn pid_to_node(pid: u32) -> u8 {
    (pid >> 24) as u8
}

/// Extract the local PID from a global PID.
pub fn pid_to_local(pid: u32) -> u32 {
    pid & 0x00FF_FFFF
}

/// Get the global PID for the calling process.
...
/// Request the process list from a remote node.
pub fn remote_proclist(
    node_id: u32,
    out_buf: &mut [protocol::RemoteProcEntry],
    ...
    loop {
        ...
        for i in 0..count {
            if buf[i].valid {
                if let Some(count_out) = parse_proclist_resp(
                    &buf[i].payload[..buf[i].payload_len],
                    out_buf,
                ) {
                    return Ok(count_out);
                }
...
/// Handle an incoming PROCLIST_REQ — serialize local procs and respond.
fn handle_proclist_request(src_node: u8) {
    ...
    for proc in procs.iter() {
        ...
        let name = b"proc";
        raw_entries[count] = (proc.pid, state_byte, name);
        count += 1;
    }
    drop(procs);
...
// ── Transport: process incoming messages ────────────────────────────────────────

/// Process an incoming distributed IPC message on our well-known endpoint.
pub fn process_incoming_message(msg: &Message) {
    ...
    match magic {
        MAGIC_PING => {
            if let Some(src_node) = parse_ping(buf) {
                handle_incoming_ping(src_node);
            }
        }
        MAGIC_PONG => {
            if let Some(src_node) = parse_pong(buf) {
                handle_incoming_pong(src_node);
            }
        }
        MAGIC_DATA => {
            if let Some((src_node, remote_ep, payload)) = parse_data(buf) {
                queue_incoming_message(src_node, remote_ep, payload);
            }
        }
        MAGIC_CAP => {
            let pid = crate::sched::current_thread()
            ...
            if cnode_id > 0 {
                let _ = cap_receive(cnode_id, buf);
            }
        }
        MAGIC_PROCLIST_REQ => {
            if let Some(requester) = parse_proclist_req(buf) {
                handle_proclist_request(requester);
            }
        }
        _ => {}
    }
...
/// Register a remote node by IP address and port.
...
pub fn publish_endpoint(local_ep: usize, remote_node: u32, remote_ep: usize) -> bool {
    ...
    if count >= MAX_REMOTE_EPS {
        return false;
    }

    eps[count] = RemoteEndpoint {
        ep_id: local_ep,
        remote_node,
        remote_ep,
        registered: true,
...
/// Get the number of registered remote nodes.
pub fn remote_node_count() -> usize {
    *REMOTE_NODE_COUNT.lock()
}

/// Check if a remote node is online.
...
/// Get node info for a given node_id.
